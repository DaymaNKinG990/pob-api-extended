...
class ConfigBuilder:
    ...
    def build(xml_root, character_level: int) -> config.Config:
        """
        Build a config.Config from an XML root using mapped Input entries and the provided character level.
        
        Parameters:
            xml_root (xml.etree.ElementTree.Element): XML root element expected to contain an optional "Config" child with "Input" elements.
            character_level (int): Character level to include in the resulting Config; used when the "Config" section is absent and always set on the returned object.
        
        Returns:
            config.Config: A Config populated from mapped input fields found under the "Config" element; any unmapped inputs are ignored and missing fields use defaults, with `character_level` set to the provided value.
        """
        ...
        def _convert_fields(item):
            ...
            Parameters:
                item (dict): A mapping that may contain the keys "boolean", "number", or "string".
                    - If "boolean" is present, the boolean value is represented (always `True` here).
                    - If "number" is present, its value is parsed as an integer.
                    - If "string" is present, its value is returned with the first character capitalized.
            
            Returns:
                The converted value: `True` if "boolean" is present, an `int` for "number",
                a capitalized `str` for "string", or `None` if none of those keys are present.
            """
            if item.get("boolean"):
                return True
...
class ItemSetBuilder:
    ...
    def build_all(xml_root) -> list[models.Set]:
        ...
        Returns:
            A list of models.Set instances constructed from the item sets found in the XML.
        """
        return [
    ...
    def _build_single(item_set_data: dict) -> models.Set:
        """
        Constructs a models.Set with all equipment and flask slots present; any missing fields in item_set_data are set to None.
        
        Parameters:
            item_set_data (dict): Mapping of Set field names to values; provided keys override defaults.
        
        Returns:
            models.Set: A Set instance populated with the provided values and None for any absent fields.
        """
